...
export CONTAINER_NAME=$(basename $PWD-prediction-20)
export USER_ID=${UID}
...
-w /workspace \
${DOCKER_IMAGE} \
/bin/bash -c "export CUDA_VISIBLE_DEVICES=4; export TF_CUDNN_WORKSPACE_LIMIT_IN_MB=2000; export PYTHONPATH=${GUNPOWDER_PATH}:${SIMPLEFERENCE_PATH}:${Z_PATH}:\$PYTHONPATH; python -u ${PRED_PATH}/run_inference.py 20"
