...
USER_ID=${UID}
CONTAINER_NAME=$(basename ${USER}-${WORK_DIR}-prediction-2-${RANDOM})
...
-w /workspace \
${DOCKER_IMAGE} \
/bin/bash -c "sleep 5; export TF_CUDNN_WORKSPACE_LIMIT_IN_MB=2000; export PYTHONPATH=${GUNPOWDER_PATH}:${SIMPLEFERENCE_PATH}:${Z_PATH}:\$PYTHONPATH; python -u ${PRED_PATH}/run_inference.py 2"
